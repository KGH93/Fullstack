...
public class MemberController {
    ...
    public String submitAddNewMember(@Valid MemberFormDto memberFormDto,
                                     BindingResult bindingResult, Model model) { //bindingresult dto에있는 유효성검사를 검열해주는 기능
        if (bindingResult.hasErrors()) {
            return "member/addMember";
        }
        try {
            Member member = Member.createMember(memberFormDto, passwordEncoder);  //passwordencoder 쓰려면 위에서 선언필요
    ...
    public String requestUpdateMemberForm(@PathVariable(name = "memberId") String memberId,
                                          Model model) {
        System.out.println(">>> memberId 파라미터: " + memberId);

        Member member = memberService.getMemberById(memberId);

        if (member == null) {
            System.out.println(">>> memberService.getMemberById 결과: null");
        } else {
            System.out.println(">>> 불러온 Member 객체: " + member);
        }
