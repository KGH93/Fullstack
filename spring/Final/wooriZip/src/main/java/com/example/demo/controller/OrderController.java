...
public class OrderController {
    ...
    @PostMapping("/remove")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> removeOrderItem(@RequestBody Map<String, Object> payload, Principal principal) {
        ...
        try {
            orderService.removeOrderItem(orderId, orderItemId, userId);
...
//        model.addAttribute("orders", order);
    ...
    // 바로구매 처리
    @PostMapping("/now")
    public String orderNow(@RequestParam Long productId, CartDto dto , Principal principal, Model model) {
        log.info("바로구매로 넘어온 값 : 상품아이디 : {} , dto {}", productId, dto.toString());
        String id = principal.getName();

        OrderDto orderDto = orderService.createOrderByNow(dto, productId, id);
        model.addAttribute("orderDto", orderDto);
        return "redirect:/order";
    }
