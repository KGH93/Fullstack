...
public class UserService implements UserDetailsService {
    ...
    //Spring Security 가 로그인시 호출하는 메서드 반드시 오버라이드 해야함
    @Override
    ...
    public void userCheck(UserPwRequestDto requestDto) {
        ...
        //requestDto에 들어있는 userId 를 조회하고
        //그회원의 이메일 requestDto셋팅
        requestDto.setUserEmail(user.getEmail());
        sendEmail(requestDto);
        //sendEmail 메서드 호출해서 비밀번호 재성정 메일을 전송
    }
...
//Users 직적만든 엔티티
// DB 저장용, 회원정보관리
// Db 에서 조회한 후  User로 변환

//User(Security 에서 제공)
//인증용 Spring Security 로그인처리 -> 인증 시스템 전달용
//principal.getName();  해서 시큐리티 로그인한 사용자 불러올수 있다.
