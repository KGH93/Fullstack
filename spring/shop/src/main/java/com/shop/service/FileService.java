...
//uploadPath : 저장할 경로 itemImgLocation에서 넘어옴
    public String uploadFile(String uploadPath, String originalFileName, byte[] fileData) throws Exception{
        UUID uuid = UUID.randomUUID();  //랜덤한 고유 id 생성
        String extension = originalFileName.substring(originalFileName.lastIndexOf(".")); //확장자만 추출
        String savedFileName = uuid.toString() + extension; //고유id에 확장자를 더해서 저장할 파일명 생성
        String fileUploadFullUrl = uploadPath + "/" + savedFileName;  //d:/shop/item/asdfklsjdf.jpg
