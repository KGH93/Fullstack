...
function App() {
  ...
  // 처음랜더링 될때 한번만 fetchTodos() -> 전체리스트를 서버에서 호출
  useEffect(() => {
    fetchTodos();
  }, []);

  // 추가
  // 텍스타가 공백이 아니면, 새로운 할일을 서버에 post 요청으로 추가
  const addTodo = async () => {
    if (!text.trim()) return;
    await axios.post('http://localhost:5000/todos', { text })
        setText('');  //성공적으로 추가되면, 입력란을 비운다.
        fetchTodos();
  };
  ...
  return (
    <div style={{padding:'2rem'}}>  
         ...
         <ul>
            {todos.map((todo) => (
              <li key={todo.id}>
                <span style={{ textDecoration: todo.completed ? 'line-through' : 'none', marginRight: '1rem', cursor: 'pointer' }} 
