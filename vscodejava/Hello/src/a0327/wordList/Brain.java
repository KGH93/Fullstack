...
public class Brain implements Interface {
        ...
        while (true) {
            ...
            if(Eng.equals("Menu") || Eng.equals("menu")){
                ...
                if(word.containsKey(Eng) == true){
                    System.out.println("이미입력된단어입니다. 수정을 원하시면 삭제후 다시입력");
                }else{
    ...
    public void WordSearch() {
       while (true) {
          Scanner scan = new Scanner(System.in);
          System.out.println("조회하고싶은 영단어를 입력하세요. (목록으로 돌아가려면 Menu입력)");
          String Eng  =  scan.nextLine(); 
          if(Eng.equals("Menu") || Eng.equals("menu")){
            break;
          }else{
            if (word.containsKey(Eng) == true) {
                System.out.println(word.get(Eng)); //key 해당되는 value(단어뜻)
            }else{
   ...
   @Override
    public void DeleteWord() {
        ...
        if(del.equalsIgnoreCase("menu")){//대소문자 구분 없이 비교
    ...
    @Override
    public void Test() {
        Scanner scan = new Scanner(System.in); // Scanner 반복문 밖에서 생성
        while (true) {
